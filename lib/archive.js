...
var AsarArchive, MAX_SAFE_INTEGER, crypto, fs, minimatch, mkdirp, os, path, queue, sortBy, stream, uint64le, walkdir, zlib,
...
module.exports = AsarArchive = (function() {
  ...
  AsarArchive.prototype._writeFooter = function(out, cb) {
    ...
    headerSizeBuf = uint64le.numberToBuffer(this._headerSize);
      ...
      return function() {
        out.write(headerSizeBuf, function() {
          ...
          archiveFile.on('end', function() {
            ...
            archiveSizeBuf = uint64le.numberToBuffer(_this._archiveSize);
  ...
  AsarArchive.prototype.write = function(archiveName, opts, cb) {
    ...
    writeFile = (function(_this) {
      return function(filename, out, internalFilename, node, cb) {
        var gzip, realSize, src, _base;
        if (typeof (_base = _this.opts).onFileBegin === "function") {
          _base.onFileBegin(path.sep + internalFilename);
        }
        realSize = 0;
        src = fs.createReadStream(filename);
        if (_this.opts.compress && node.size > _this.opts.minSizeToCompress) {
          if (_this.opts.onProgress != null) {
            src.on('data', function(chunk) {
              var _base1;
              if (typeof (_base1 = _this.opts).onProgress === "function") {
                _base1.onProgress(_this._filesSize, chunk.length, internalFilename);
              }
            });
          }
  ...
  AsarArchive.prototype.extract = function(dest, opts, cb) {
    var archiveRoot, destDir, destFilename, extractSize, filename, filenames, linkTo, linkToRel, node, pattern, q, relativeTo, symlinksSupported, writeStreamToFile, _i, _j, _len, _len1, _ref, _ref1;
    if (typeof opts === 'function') {
      cb = opts;
      opts = {};
    }
    ...
    if (this.opts.onProgress != null) {
      extractSize = 0;
      for (_i = 0, _len = filenames.length; _i < _len; _i++) {
        filename = filenames[_i];
        extractSize += this._searchNode(filename).size || 0;
      }
    }
    ...
    writeStreamToFile = (function(_this) {
      return function(filename, destFilename, cb) {
        var inStream, out, _base;
        if (typeof (_base = _this.opts).onFileBegin === "function") {
          _base.onFileBegin(destFilename);
        }
        inStream = _this.createReadStream(filename);
        if (_this.opts.onProgress != null) {
          inStream.on('data', function(chunk) {
            var _base1;
            return typeof (_base1 = _this.opts).onProgress === "function" ? _base1.onProgress(extractSize, chunk.length, filename) : void 0;
          });
        }
