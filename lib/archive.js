...
var AsarArchive, MAX_SAFE_INTEGER, UINT64, crypto, fs, glob, minimatch, mkdirp, origFs, os, path, queue, readUINT64, sortBy, stream, walkdir, writeUINT64, zlib,
...
module.exports = AsarArchive = (function() {
  ...
  AsarArchive.prototype.getFile = function(filename) {
    ...
    fs.closeSync(fd);
    if (node.csize) {
      if (zlib.gunzipSync == null) {
        throw new Error('Synchronous decompression not supported in node < 0.11\nYou can try node-zlib-backport for node 0.10.x');
      }
